...
BitcoinMinerHelper::BitcoinMinerHelper (std::string protocol, Address address, std::vector<Ipv4Address> peers, int noMiners,
                                        ...
                                        nodeInternetSpeeds &internetSpeeds, nodeStatistics *stats, double hashRate, double averageBlockGenIntervalSeconds,int noMajoriry = 0) : 
                                        ...
                                        m_secureBlocks (6), m_blockGenBinSize (-1), m_blockGenParameter (-1)
{
  ...
  m_majorityCount = noMajoriry;
  m_factory.Set("MajorityCount",UintegerValue(m_majorityCount));
...
BitcoinMinerHelper::InstallPriv (Ptr<Node> node) //FIX ME
{
   ...
   switch (m_minerType) 
   {
      case NORMAL_MINER: 
      {
		...
		app->SetDagViews(m_dagViews,m_dagStats);
		//app->SetMajorityCount(m_majorityCount);
