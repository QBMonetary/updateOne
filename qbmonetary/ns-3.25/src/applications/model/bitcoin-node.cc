...
bool g_debug = false;
#define COUT_FOR_NODE if (-1 != GetNode()->GetId() && g_debug)\
	 std::cout
...
#ifdef EXPLORER_TEST
    .AddAttribute ("IsExplorer",
                   "If the node is for block explorer",
                   MakeBooleanAccessor (&BitcoinNode::m_explorer),
                   MakeBooleanChecker ())
#endif
    .AddTraceSource ("Rx",
...
void 
BitcoinNode::SetDagViews (std::string *dagViews,std::string *dagStats)
{
  NS_LOG_FUNCTION (this);
  //std::cout << "BitcoinNode::SetDagViews" <<dagViews<<"\n";
  m_dagViews = dagViews;
  m_dagStats = dagStats;
}
...
BitcoinNode::StartApplication ()    // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
  /* BEGIN: Added by xuzt, 2019/5/14   DAG simulator support */
  m_blockchain.m_nodeId = GetNode()->GetId();
  /* END:   Added by xuzt, 2019/5/14 */
...
/* BEGIN: Added by xuzt, 2019/5/27   DAG simulator support */
struct BlockPrintCompare {
    bool operator()(std::string hashFirst,std::string hashSecond)
    {
        int heightFirst = std::stoi(hashFirst.substr(0,hashFirst.find("/")));
        int heightSecond = std::stoi(hashSecond.substr(0,hashSecond.find("/")));
...
BitcoinNode::StopApplication ()     // Called at time specified by Stop
{
  ...
  //Block bestFreeBlock = m_blockchain.GetBestFreeBlock();
  //if (bestFreeBlock.GetBlockHeight() < 0)return;
  //printf("Best free block is %s\n",bestFreeBlock.GetBlockHash().c_str());
  //std::cout<<"m_dagViews = "<<m_dagViews<<",m_dagStats = "<<m_dagStats<<"\n";
  std::string outStr = "";
  std::vector<std::string> stableChain = m_blockchain.m_stableChain;
  std::string lastStableHash = stableChain[stableChain.size() - 1];
  std::queue<std::string> stables;
  std::unordered_set<std::string> handledHashes;
  stables.push(lastStableHash);
  std::string stableHash = lastStableHash;
  // while(!stables.empty())
  // {
  //     stableHash = stables.front();
	...
	//   Block stableBlock = m_blockchain.ReturnBlock(stableHash);
  ...
  // LZ: rewrite above BFS
  handledHashes.insert(lastStableHash);
  while (!stables.empty()) {
    stableHash = stables.front();
    stables.pop();
    outStr += stableHash + ",";
    Block stableBlock = m_blockchain.ReturnBlock(stableHash);
    for (auto &parent : stableBlock.m_DagParents) {
      if (!handledHashes.count(parent)) {
        stables.push(parent);
        handledHashes.insert(parent);
      }
    }
  }

  *m_dagViews = outStr;
  //std::map<std::string,std::string,BlockPrintCompare> stablePrint;
  /*for(auto it = stableChain.begin();it != stableChain.end();it++)
  {
      Block stableBlock = m_blockchain.ReturnBlock(*it);
	  std::ostringstream stringStream;  
      //std::string stableTimeStr;
	  double stableTime = stableBlock.m_timeStable - stableBlock.GetTimeCreated();
	  
	  //if (stableTime > 0.0)
	  {
	      //std::cout<<"Stable block "<<*it<<" ,stable time "<<stableTime<<"\n";
          stringStream << stableTime;
          outStr += (*it) + " [" + stringStream.str() + "],";
		  //stablePrint[*it] = stringStream.str();
	  }
  }*/

  outStr= "";
  Block lastStableBlock = m_blockchain.ReturnBlock(lastStableHash);
  int lastStableHeight = lastStableBlock.GetBlockHeight();
  std::map<int,int> durationAdvancedNumber;
  //std::cout<<"Last stable block height is "<<lastStableHeight<<"\n";
  for (int i = 0; i <= lastStableHeight;i++)
  {
    for (int j = 0; j < m_blockchain.m_blocks[i].size();j++)
    {
      Block stableBlock = m_blockchain.m_blocks[i][j];
      if (stableBlock.m_timeStable > 0.0)
      {
        std::ostringstream stringStream;  
        double stableTime = stableBlock.m_timeStable - stableBlock.GetTimeCreated();
        stringStream << stableTime;
        outStr += stableBlock.GetBlockHash() + " ," + stringStream.str() + ",";
        // LZ: remove m_bpIncludedMci
        // if (stableBlock.m_isOnMainChain)
        //   outStr += std::to_string(stableBlock.m_mci);
        // else
        //   outStr += std::to_string(stableBlock.m_bpIncludedMci);
        outStr += std::to_string(stableBlock.m_mci);
        outStr += ",;";

        int duractionIndex = stableBlock.m_timeStable / m_checkDuration;
        durationAdvancedNumber[duractionIndex]++;
      }
    }
  }
  outStr += "------\n";
  // for (int i = 0;i < durationAdvancedNumber.size();i++)
  // {
	//   int totalNumber = 0;
	//   for (int j = 0; j <= i;j++)
	//   	totalNumber += durationAdvancedNumber[j];
	//   //durationAdvancedNumber[i] = totalNumber;
	//   //double durationStart = index * m_checkDuration;
	//   double durationEnd = ( i + 1 ) * m_checkDuration;
	//   //std::ostringstream stringStream;
	//   char buf[64] = {0};
	//   sprintf(buf,"%f,%d,;",durationEnd,totalNumber);
	//   std::string durationStr(buf);
	//   outStr += durationStr;
  // }

  // LZ: rewrite the above
  int totalNum = 0;
  for (auto p : durationAdvancedNumber) {
    totalNum += p.second;
    double durationEnd = (p.first + 1) * m_checkDuration;
    char buf[64] = {0};
	  sprintf(buf,"%f,%d,;", durationEnd, totalNum);
	  std::string durationStr(buf);
	  outStr += durationStr;
  }
  *m_dagStats = outStr; 
...
//add by xuzt
#ifdef EXPLORER_TEST
bool BitcoinNode::isExplorerNode()
{
    return m_explorer;
}

bool BitcoinNode::ZmqInit()
{
    pZmqContent = zmq_init(1);
	if (!pZmqContent)
	{
	    printf("Unable to initialize context\r\n");
		return false;
	}
    pZmqSocket = zmq_socket(, ZMQ_PUB);
    if (!pZmqSocket)
    {
    ...
    int rc = zmq_bind(pZmqSocket, "tcp://127.0.0.1:28332");
    if (rc!=0)
    {
...
void BitcoinNode::ZmqShutDown()
{
    int linger = 0;
    zmq_setsockopt(pZmqSocket, ZMQ_LINGER, &linger, sizeof(linger));
    zmq_close(pZmqSocket);
	zmq_ctx_destroy(pZmqContent);
}
int BitcoinNode::ZmqNotifyBlock(Block& block)
{
	NS_LOG_FUNCTION (this);
	char[64] data = {0};
	sprintf(&data[0],"blockhash%d %d %d %.2f",block.GetBlockHeight(),block.GetMinerId(),block.GetBlockSizeBytes(void),block.GetTimeCreated());
	zmq_msg_t msg;
	
	int rc = zmq_msg_init_size(&msg, strlen(data));
	if (rc != 0)
	{
		printf("Unable to initialize ZMQ msg\r\n");
		return -1;
	}

	void *zbuf = zmq_msg_data(&msg);
	memcpy(zbuf, data, strlen(data));

	rc = zmq_msg_send(&msg, sock, 0);
	if (rc == -1)
	{
		printf("Unable to send ZMQ msg\r\n");
		zmq_msg_close(&msg);
		return -1;
	}
              ...
              if (!requestBlocks.empty())
              {
                ...
                for (block_it = requestBlocks.begin(); block_it < requestBlocks.end(); block_it++) 
                {
                  ...
                  blockInfo.AddMember("timeReceived", value, d.GetAllocator ());
				          ...
				          /* BEGIN: Added by xuzt, 2019/4/23   DAG simulator support */
            	    int index = 0;
				          rapidjson::Value pblockArray(rapidjson::kArrayType);
                  for (auto &pblock : block_it->GetDagParents())
                  {
                      NS_LOG_DEBUG ("Time " << Simulator::Now ().GetSeconds () << ": Miner " << GetNode ()->GetId ()
            					<< " block rapidjson set dag parent hash [" << index << "]:" << pblock);
                      value.SetString(pblock.c_str(),d.GetAllocator());
                      pblockArray.PushBack(value,d.GetAllocator());
            		      index++;
                  }
                  blockInfo.AddMember("dagParents",pblockArray,d.GetAllocator());
            
                  // LZ: remove
                  // value = block_it->m_witnessLevel;
                  // blockInfo.AddMember("witnessedLevel", value, d.GetAllocator ());
              
                  value = block_it->m_lastStableMci;
                  //std::cout<<"Add block last stable mci is "<<block_it->m_lastStableMci<<"\n";
                  blockInfo.AddMember("lastStableMci", value, d.GetAllocator ());
                  /* END:   Added by xuzt, 2019/4/23 */
                  array.PushBack(blockInfo, d.GetAllocator());
                }	
  ...
  {  
    ...
    {*/
      Block newBlock (d["blocks"][j]["height"].GetInt(), d["blocks"][j]["minerId"].GetInt(), d["blocks"][j]["parentBlockMinerId"].GetInt(), 
                      d["blocks"][j]["size"].GetInt(), d["blocks"][j]["timeCreated"].GetDouble(), 
                      Simulator::Now ().GetSeconds (), InetSocketAddress::ConvertFrom(from).GetIpv4 ());
	  std::vector<std::string> dagParents;
	  for (auto &v : d["blocks"][j]["dagParents"].GetArray())
	  	  dagParents.push_back(v.GetString());
      newBlock.SetDagParents(dagParents);	  
      // newBlock.m_witnessLevel = d["blocks"][j]["witnessedLevel"].GetInt(); // LZ: remove
	    newBlock.m_lastStableMci = d["blocks"][j]["lastStableMci"].GetInt();
...
BitcoinNode::ReceiveBlock(const Block &newBlock) 
{
  ...
  stringStream << newBlock.GetBlockHeight() << "/" << newBlock.GetMinerId() <<"/" << newBlock.GetTimeCreated();
  blockHash = stringStream.str();

  //if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock) || ReceivedButNotValidated(blockHash))
  if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock))
  {
...
//   NS_LOG_FUNCTION (this);
//   /* BEGIN: Modified by xuzt, 2019/4/19   DAG simulator support */
//   //1.validate block info

//   //2.validate parents
//   //const Block *parent = m_blockchain.GetParent(newBlock);
//   COUT_FOR_NODE<<"ValidateBlock "<<newBlock<<"\n";
//   bool hasParent = m_blockchain.CheckDagParents(newBlock);
  
//   //if (parent == nullptr)
//   if (!hasParent)
//   {
//     NS_LOG_INFO("ValidateBlock: Block " << newBlock << " is an orphan\n"); 
// 	COUT_FOR_NODE<<"BitcoinNode::ValidateBlock [missing] Now the main chain is :\n";
// 	for (auto &mc : m_blockchain.m_mainChain)
// 	    COUT_FOR_NODE<<mc<<",";
// 	COUT_FOR_NODE<<"\n";
// 	 m_blockchain.AddOrphan(newBlock);
// 	 for (auto& missingParent : m_blockchain.m_missingParents[newBlock.GetBlockHash()])
// 	 {
// 	     Block missingBlock = m_blockchain.ReturnBlock(missingParent);
// 	     if (m_blockchain.IsOrphan(missingBlock))
// 		 	continue;
...
//          value.SetString(missingParent.c_str(), missingParent.size(), d.GetAllocator());
//          array.PushBack(value, d.GetAllocator());
//          d.AddMember("blocks", array, d.GetAllocator());
     
// 	     //int peer = rand() % (m_peersAddresses.size());
// 		 //auto it = m_peersAddresses.begin() + peer;
// 		 NS_LOG_INFO("ValidateBlock: Request missing parent " << missingParent <<"from ip "<<newBlock.GetReceivedFromIpv4() << " \n");
// 		 InetSocketAddress inetAddress(newBlock.GetReceivedFromIpv4());
// 	     SendMessage(INV, GET_DATA, d, m_peersSockets[newBlock.GetReceivedFromIpv4()]);
// 	 }
// 	 //m_blockchain.PrintOrphans();
//   }
//   /* END:   Modified by xuzt, 2019/4/19 */
...
//     NS_LOG_INFO("ValidateBlock: Block's " << newBlock << " parent is :\n");
//     for (auto const& parent:newBlock.GetDagParents())
//     {
//        NS_LOG_INFO("" << parent <<"\n");     
//     }
...
//      * Block is not orphan, so we can go on validating
//      */	
// 	/* BEGIN: Added by xuzt, 2019/5/8   DAG simulator support */
// 	Block bpBlock = m_blockchain.GetBestParent(newBlock);
//     //std::cout<<"ValidateBlock get best parent is "<<bpBlock.GetBlockHash()<<"\n";
// 	//best parent not found
// 	if (bpBlock.GetBlockHeight() < 0)
// 		return;

// 	//update parent block's children
//     //m_blockchain.UpdateChildren(newBlock);
// 	//child block's witness level less than parent's
// 	//int witnessLevel = m_blockchain.GetWitnessLevel(newBlock);
// 	COUT_FOR_NODE<<"new block's witness level is "<< newBlock.m_witnessLevel <<" and best parents's is "<<bpBlock.m_witnessLevel<<"!!!!\n";
// 	if (newBlock.m_witnessLevel < bpBlock.m_witnessLevel)
// 	{
// 	    COUT_FOR_NODE<<"new block's witness level is less than the best parents's!!!!\n";
// 		return;
// 	}

	
// 	//3.validate last stable block
// 	int lsbMci = newBlock.m_lastStableMci;
// 	//COUT_FOR_NODE<<"Last stable mci is "<<lsbMci<<"\n";
// 	Block lsb;
// 	int maxParentLsbMci = 0;
// 	COUT_FOR_NODE<<"BitcoinNode::ValidateBlock [missing]  Now the main chain is :\n";
// 	for (auto &mc : m_blockchain.m_mainChain)
// 		COUT_FOR_NODE<<mc<<",";
// 	COUT_FOR_NODE<<"\n";
// 	if (lsbMci < 0 || lsbMci > m_blockchain.m_mainChain.size())
// 	{
// 	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable mci "<<lsbMci<<" is invalid \n";
// 		return;
// 	}
// 	lsb = m_blockchain.ReturnBlock(m_blockchain.m_mainChain[lsbMci]);
// 	if (lsb.GetBlockHeight() == -1 || lsb.m_isOnMainChain == false )
// 	{
// 	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable block "<<lsb.GetBlockHash()<<" is not found or on main chain, "\
// 			<<lsb.m_isOnMainChain<<","<<lsb.GetBlockHeight()<<"\n";
// 		return;
// 	}
// 	for (auto& phash : newBlock.GetDagParents())
// 	{
// 	    Block pblock = m_blockchain.ReturnBlock(phash);
// 		//if (pblock.GetBlockHeight() == -1)
// 		if (pblock.m_lastStableMci > maxParentLsbMci)
// 			maxParentLsbMci = pblock.m_lastStableMci;
// 	}

// 	if ( lsbMci < maxParentLsbMci )
// 	{
// 	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable mci = "<<lsbMci<<"< maxParentLsbMci = "<<maxParentLsbMci<<"\n";
// 		return;
// 	}
// 	if (!m_blockchain.CheckStable(maxParentLsbMci, lsbMci, newBlock))
// 	{
// 	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable mci "<<lsbMci<<","<<maxParentLsbMci<<" check is not stable \n";
// 		return;
// 	}

// 	if (lsb.m_timeStable - 0.0 <= 0.0000001)
// 	    lsb.m_timeStable = Simulator::Now ().GetSeconds ();
// 	m_blockchain.UpdateBlock(lsb);
    
// 	/*if ( m_blockchain.CheckStable(maxParentLsbMci, lsbMci + 1, newBlock) )
// 	{
// 	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" next last stable mci "<<lsbMci+1<<","<<maxParentLsbMci<<" check is stable \n";
// 		return;
// 	}*/
	
// 	/* END:   Added by xuzt, 2019/5/8 */
...
BitcoinNode::ValidateBlock(const Block &newBlock) 
{
  ...
  if (!hasParent)
  {
    ...
    // std::cout << GetNode()->GetId() << std::endl; // LZ: tmp
  ...
  else 
  {
    ...
    Block lsb = m_blockchain.ReturnBlock(m_blockchain.m_mainChain[lsbMci]);
	  if (lsb.GetBlockHeight() == -1 || lsb.m_isOnMainChain == false)
	  {
	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable block "<<lsb.GetBlockHash()<<" is not found or on main chain, "
			<<lsb.m_isOnMainChain<<","<<lsb.GetBlockHeight()<<"\n";
    ...
    if (m_blockchain.getLsbMci(newBlock) != lsbMci)
	  {
	    COUT_FOR_NODE<<"New block "<<newBlock.GetBlockHash()<<" last stable mci "<<lsbMci<<" check is not stable \n";

      // LZ: debug info
      std::cout<<GetNode()->GetId() << ": New block "<<newBlock.GetBlockHash()<<" last stable mci "<<lsbMci<<" check is not stable \n"; 
      std::cout << m_blockchain.getLsbMci(newBlock) << " " << lsbMci << std::endl;
		  return;
	  }
...
/* END:   Modified by xuzt, 2019/4/23 */
  ...
  //std::cout<<"After block validation add new block "<<newBlock.GetBlockHash()<<" to block chain :\n"<<m_blockchain<<"\n";
				  
  m_blockchain.AddBlock(newBlock);

  // LZ: debug info
  // Block block = m_blockchain.ReturnBlock(newBlock.GetBlockHash());
  // std::cout << "Node " << GetNode()->GetId() << " receive block " << block.GetBlockHash() << " with m_bpIncludedMci size " 
  // << block.m_bpIncludedMci.size() << std::endl;

  
  /* BEGIN: Added by xuzt, 2019/4/23   DAG simulator support */
  //std::cout<<"After block validation Add free block "<<newBlock.GetBlockHash()<<"\n";

  /* END:   Added by xuzt, 2019/4/23 */

  /* BEGIN: Deleted by xuzt, 2019/4/23   DAG simulator support */
  /*if (!m_blockTorrent)
    AdvertiseNewBlock(newBlock); 
  else
    AdvertiseFullBlock(newBlock);*/
  /* END: Deleted by xuzt, 2019/4/23 */
  ...
  if (isExplorerNode())
  {
      //notify block explorer through zmq
      ZmqNotifyBlock(newBlock);
  }
  #endif
}  
...
BitcoinNode::ValidateOrphanChildren(const Block &newBlock) 
{
  ...
  std::vector<Block> children = m_blockchain.CheckMissingParent(newBlock.GetBlockHash()); 
  if (children.size() == 0)
  {
