...
public:
  ...
  */
  ...
  void SetDagViews(std::string *dagViews,std::string *dagStats);
...
protected:
  ...
  #ifdef EXPLORER_TEST
  bool isExplorerNode();
  int ZmqNotifyBlock(Block block);
  void ZmqShutDown();
  bool ZmqInit();
  #endif
  ...
  /* BEGIN: Added by xuzt, 2019/4/23   DAG simulator support */
  std::string                                        *m_dagViews;
  std::string                                        *m_dagStats;
  double                                              m_checkDuration;
  /* END:   Added by xuzt, 2019/4/23 */
