...
{  
  ...
  stringStream << blockHeight << "/" << minerId << "/" << timeCreated;
  m_blockHash = stringStream.str();
  m_isOnMainChain = false;
  m_mci = -1;
  // m_witnessLevel = 0; // LZ: remove
  m_lastStableMci = 0;
  //m_bpIncludedMci = 0; // LZ: remove
  // m_earliestBpIncludedMci = 0xFFFFFF; // LZ: remove
  m_timeStable = 0.0;
...
{  
  ...
  std::ostringstream stringStream;  
  std::string blockHash;
  stringStream << m_blockHeight << "/" << m_minerId << "/" << m_timeCreated;
  m_blockHash = stringStream.str();
  // m_witnessLevel = blockSource.m_witnessLevel; // LZ: remove
  m_isOnMainChain = blockSource.m_isOnMainChain;
  m_mci = blockSource.m_mci;
  m_lastStableMci = blockSource.m_lastStableMci;
  // m_bpIncludedMci = blockSource.m_bpIncludedMci; // LZ: remove
  m_bestParent = blockSource.m_bestParent;
  // m_earliestBpIncludedMci = blockSource.m_earliestBpIncludedMci; // LZ: remove
...
std::string
Block::printDagParents() const
{
  std::stringstream s;
  s << "[";
  for (auto const& parent : m_DagParents)
  {
    if (parent != m_DagParents.back())
      s << parent << ",";
    else
      s << parent;
  }
...
Blockchain::ReturnBlock(std::string blockHash)
{
  if (m_blockMap.count(blockHash))
    return m_blockMap[blockHash];
  return Block(-1, -1, -1, -1, -1, -1, Ipv4Address("0.0.0.0"));
}
...
Blockchain::UpdateBlock(Block& newBlock)
{
  ...
  m_blockMap[newBlock.GetBlockHash()] = newBlock; // LZ
	return true;
}
...
{
   int maxLevel = -1;
   std::string maxBlockHash = "";
   Block bestParent(-1, -1);
   for (Block &pblock : pblocks)
   {
      int blockHeight = pblock.GetBlockHeight();
      if (blockHeight > maxLevel || (blockHeight == maxLevel && pblock.GetBlockHash() > maxBlockHash))
      {
        maxLevel = blockHeight;
	      maxBlockHash = pblock.GetBlockHash();
		    bestParent = pblock;
      }
   }
   return bestParent;
...
// LZ: added
int 
Blockchain::getLsbMci(Block block)
{
  Block bpBlock = GetBestParent(block);
  int oldMci = bpBlock.m_lastStableMci;
  int thresholdLevel = 2 * (m_majorityCount - 1);
  if (block.GetBlockHeight() <= oldMci + thresholdLevel)
    return oldMci;

  std::unordered_set<std::string> mcBlocks;
  Block bBlock = bpBlock;
  while (true)
  {
    mcBlocks.insert(bBlock.GetBlockHash());
    if (bBlock.GetBlockHash() == m_mainChain[oldMci])
      break;
    bBlock = GetBestParent(bBlock);
  }

  std::queue<std::string> searchBlocks;
	std::unordered_set<std::string> searchedHashes;
  std::map<int, int> max_level;
  for (auto pBlock : block.m_DagParents)
  {
    searchBlocks.push(pBlock);
    searchedHashes.insert(pBlock);
  }
  while (!searchBlocks.empty()) 
  {
    std::string bHash = std::move(searchBlocks.front());
    searchBlocks.pop();
    Block bBlock = ReturnBlock(bHash), block = bBlock;
    int mci = 0;
    while (true)
    {
      if (mcBlocks.count(block.GetBlockHash()))
      {
        mci = block.GetBlockHeight();
        break;
      }
      if (block.m_isOnMainChain && block.GetBlockHeight() < oldMci)
      {
        mci = -1;
        break;
      }
      block = ReturnBlock(block.m_bestParent);
    }
      
    if (mci < oldMci)
      continue;
    if (!max_level.count(mci))
      max_level[mci] = bBlock.GetBlockHeight();
    else
      max_level[mci] = std::max(max_level[mci], bBlock.GetBlockHeight());
    for (auto pBlock : bBlock.GetDagParents())
    {
      if (!searchedHashes.count(pBlock))
      {
        searchBlocks.push(pBlock);
        searchedHashes.insert(pBlock);
      }
	...
	for (int i = oldMci; i < block.GetBlockHeight(); i++)
  {
    // LZ: debug info
    // if (!max_level.count(i)) 
    // {
    //   std::cout << max_level.size() << std::endl;
    //   for (auto p : max_level)
    //     std::cout << p.first << ",";
    //   std::cout << std::endl;
    //   for (auto bHash : m_mainChain)
    //     std::cout << bHash << ",";
    //   std::cout << std::endl;
    // }

    int k = max_level.count(i) ? max_level[i] : i;
    if (block.GetBlockHeight() <= k + thresholdLevel) 
      return i;
  }    
  return oldMci;
}
...
// 	    //int freeBlocksLen = m_freeBlocks.size();
// 		std::string pickHash = m_freeBlocks[randIndex].GetBlockHash();
// 		//Block pickParent = ReturnBlock(pickHash);
// 		COUT_FOR_NODE<<"Pick the free parent = "<<pickHash<<"\n";
// 		dagParentBlocks.push_back(pickHash);
...
Blockchain::pickParents(Block& newBlock)
{
  ...
  // 1. pick best free tip block as best parent
	std::string bestFreeHash = GetBestFreeBlock();
	Block bestFreeBlock = ReturnBlock(bestFreeHash);
	COUT_FOR_NODE<<"Pick the tip block is "<<bestFreeBlock.GetBlockHash()<<"\n";

  // 2. check Assumption A4
  std::unordered_set<int> witness_list;
  witness_list.insert(newBlock.GetMinerId());
  Block bpBlock = bestFreeBlock;
  while (witness_list.size() < m_majorityCount) 
  {
    if (bpBlock.GetBlockHeight() == 0)
      break;
    auto p = witness_list.insert(bpBlock.GetMinerId());
    if (!p.second)
      return false;
    bpBlock = ReturnBlock(bpBlock.m_bestParent);
  }
  std::vector<std::string> dagParentBlocks;
  dagParentBlocks.push_back(bestFreeHash);
	RemoveFree(bestFreeBlock);
  newBlock.SetBlockHeight(bestFreeBlock.GetBlockHeight() + 1); 
  newBlock.SetBestParent(bestFreeHash);

  // 3. pick the previous block as one of the 3 non-best parents
  int num = 3;
  if (!m_previousBlock.empty())
  {
    num--;
    dagParentBlocks.push_back(m_previousBlock);
    RemoveFree(ReturnBlock(m_previousBlock));
  }
  m_previousBlock = newBlock.GetBlockHash();
  
	// 3. pick free blocks at random as non-best parents
  int i = 0;
	while (i < num && m_freeBlocks.size() > 0)
	{
	  int randIndex = rand() % m_freeBlocks.size();
		std::string pickHash = m_freeBlocks[randIndex].GetBlockHash();
		COUT_FOR_NODE<<"Pick the free parent = "<<pickHash<<"\n";
  ...
  // 4. get last stable block
  newBlock.m_lastStableMci = getLsbMci(newBlock);
...
Blockchain::UpdateMainChain(Block newBlock)
{
  ...
  assert(mci < m_mainChain.size());
	while (mci > retreatMci)
	{
	  Block oldMciBlock = ReturnBlock(m_mainChain[mci]);
	  oldMciBlock.m_isOnMainChain = false;
		UpdateBlock(oldMciBlock);
		m_mainChain.pop_back();
	  mci--;
  ...
  int newMci = retreatMci;
	...
	// update lastest included mci
	// UpdateLastestIncludedMci(retreatMci, m_lastMci, newBlock); // LZ: remove 
...
Blockchain::SaveBlock(Block newBlock)
{
	...
	// 1. update parent block's children
  UpdateChildren(updateBlock);
  ...
  UpdateMainChain(updateBlock);
	...
	// 5. update stable blocks
	bool ret = AdvanceStableBlock(updateBlock);
	return ret;
}
...
Blockchain::AdvanceStableBlock(Block& newBlock)
{
  ...
  // LZ: debug info
  // std::cout << "Blockchain::AdvanceStableBlock newBlock.m_lastStableMci is "<<newBlock.m_lastStableMci\
	//         <<", m_lastStableMci is "<<m_lastStableMci<< std::endl;
...
Blockchain::AddBlock (const Block& newBlock)
{
  ...
  m_blockMap[newBlock.GetBlockHash()] = newBlock; // LZ: add
  SaveBlock(newBlock);
  /* END:   Added by xuzt, 2019/4/19 */
}
...
std::ostream& operator<< (std::ostream &out, Blockchain &blockchain)
{
  ...
  for (blockHeight_it = blockchain.m_blocks.begin(), i = 0; blockHeight_it < blockchain.m_blocks.end(); blockHeight_it++, i++) 
  {
    ...
    {
      //out << *block_it << "\n";
      out << block_it->GetBlockHash() << "|" << block_it->printDagParents() << ", ";
    }
    out << "\n";
