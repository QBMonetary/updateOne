...
/* BEGIN: Added by xuzt, 2019/4/19   DAG simulator support */
  ...
  std::string printDagParents() const;
...
public:
  int           m_mci;
  // int           m_bpIncludedMci; // LZ: remove
  // int           m_earliestBpIncludedMci; // LZ: remove
  bool          m_isOnMainChain;
  int           m_lastStableMci;
  // int           m_witnessLevel; // LZ： remove
...
/* BEGIN: Added by xuzt, 2019/4/26   DAG simulator support */
  ...
  // static bool BlockCompare(Block a,Block b); // LZ: remove
  std::string GetBestFreeBlock();
  // int GetWitnessLevel(Block block); // LZ: remove
  // int GetMinWitnessLevel(Block block,std::vector<int>& witnessList); // LZ: remove
  bool CheckStable(int checkedBlockMci,int advancedBlockMci,Block block);
  bool pickParents(Block& newBlock);
  // bool AdjustParents(Block& newBlock); // LZ: remove
  ...
  // Block GetWitnessedLevelBlock(int minerId,std::string bestParent); // LZ: remove
  bool SetStable(std::string lsbHash,double stableTime);
  int getLsbMci(Block block); // LZ: add
...
public:
  ...
  bool                               m_debug;
  /* END:   Added by xuzt, 2019/4/24 */
  std::map<std::string, Block>       m_blockMap;      // LZ: hash -> block
  std::string                        m_previousBlock; // LZ: previous block from m_nodeId
};
